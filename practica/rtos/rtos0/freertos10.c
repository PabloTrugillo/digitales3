...
#define TAM_PILA 150
#define mainDELAY_LOOP_COUNT        ( 0xffffff )//le da el valor maximo

SemaphoreHandle_t sem_exclu;//la variable del semaforo la define como global
...
static void vTarea1(void *pvParameters)
{
  const char *pcTaskName = "Tarea 1 menos prioritaria\r\n";
   ...
   for( ;; ) {
      xSemaphoreTake( sem_exclu, portMAX_DELAY);//decrementa el sem y en caso de bloquearse, hace una espera indefinida
      ...
      xSemaphoreGive(sem_exclu);//incrementa el semaforo
...
static void vTarea2(void *pvParameters)
{
   ...
   const TickType_t xDelay200ms = pdMS_TO_TICKS( 200UL );//la conversion de ms a tiks se la asigna a la cnte q define
