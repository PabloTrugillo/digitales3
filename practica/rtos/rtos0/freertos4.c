...
void vContinuousProcessingTask( void *pvParameters )
{
   ...
   for( ;; ) {
      vPrintString( pcTaskName );//esta es lo mismo q printf solo q me da la seguridad que no se mezclen las tareas cuando acceden a la misma variable 
      //printf ( pcTaskName ); 
   }
}
/*-----------------------------------------------------------*/
...
void vPeriodicTask( void *pvParameters )
{
   ...
   for( ;; ) {
      ...
      vTaskDelayUntil( &xLastWakeTime, xDelay50ms );//aca le pasamos desde cuando tiene que dormir la tarea en el primer campo y en el segundo, cuanto tiempo debe dormir.
   }
}
