...
//Compile el programa: gcc -o final_muestra2 final_muestra2.c -lpthread -lrt
//Ejecute el programa ./final_muestra2
...
int main() {
      ...
      switch (fork())
      {
		...
		//--creo la cola de mensajes--//
		   ...
		   printf("Cola de mensajes creada\n");
			...
			er=mq_send(descriptor_mq, MENSAJE_1, sizeof(MENSAJE_1+5), 2); 
			if (er!=0){
	...
	sleep(1);
      ...
      switch (fork())
      {
			...
			sleep(1);
			descriptor_mq = mq_open(MQ_PATH, O_WRONLY);
			er=mq_send(descriptor_mq, MENSAJE_2, sizeof(MENSAJE_2+5), 1);
			if (er!=0){
	...
	sleep(2);
	 ...
	 descriptor_mq = mq_open(MQ_PATH, O_RDWR);
