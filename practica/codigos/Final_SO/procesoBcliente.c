...
Compilar: gcc -o procesoBcliente procesoBcliente.c
Ejecutar: ./procesoBcliente nro_de_puerto_de_cliente_si_es_necesario.
			Ejemplo: ./procesoBcliente 2006
...
void manejador_SIGINT(int a){			//manejador de señal SIGINT
   ...
   //printf("\nSeñal recibida por el proceso B. ");
   write (STDOUT_FILENO, "Señal recibida por el proceso B. ", 34);               	//escribe leyenda en pantalla

//****************** 3 *******************// TRANSMISION AL SERVIDOR
   
   write (sockfd , MENSAJE_B, sizeof(MENSAJE_B));		//!!! escribe en socket "CLIENTE", MENSAJE_B, tantos bytes como lo indica "sizeof()"
															//ESTO ES LO QUE RECIBE EL SERVIDOR

   lee_sck = read(sockfd , buff_sck, sizeof (MENSAJE_B));    	//lee de socket y lo guarda en el buffer "buff_sck". Es para imprimir luego por pantalla
