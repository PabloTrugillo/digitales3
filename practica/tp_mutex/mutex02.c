...
pthread_mutex_t mtx=PTHREAD_MUTEX_INITIALIZER; //aca hace la inicializacion estatica del mutex
...
int local1 , j, numero;
	...
	s=pthread_mutex_lock(&mtx);//aca resta el mutex       
	if (s!=0){
		printf("ERROR; pthread_mutex() = %d\n", s);
		exit(-1);   }
	
	printf("Hilo= %d decrementa mutex\n", numero);//imprime el valor del hilo q hace la resta al mutex
	...
	printf("Hilo= %d incrementa mutex\n", numero);//imprime el valor del hilo q hace la suma al mutex
	
	s=pthread_mutex_unlock(&mtx); //aca suma el mutex     
	if (s!=0){
		printf("ERROR; pthread_mutex() = %d\n", s);
		exit(-1);    }
