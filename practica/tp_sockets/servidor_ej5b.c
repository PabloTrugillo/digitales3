...
//#include <sys/types.h>				////Agregar
#include <pthread.h>				//Agregar

#define maxh 2

int t, rc, des_socket, sck_server, largo, cont;
pthread_t hilo[maxh];
struct sockaddr_in struct_direccion={};
int SockEscucha,SockConexion[maxh];

//Hilos para atendier client//
void *atiende (void *sfd){
	int rx_socket; 
	char buffer_rx[256];
	printf ("Desbloqueo de accept, entro conexion: %d\n",*(int*)sfd);
    send (*(int*)sfd,"Bienvenido al servidor\n", 23,0 );
	
	 while (( rx_socket = read(*(int*)sfd, buffer_rx, sizeof (buffer_rx))) > 0) {  //lee del socket    
              write (*(int*)sfd ,buffer_rx, rx_socket);         //escribe en socket
 ...
 //-- accept(): se bloquea hasta que entre una conexion --//
   ...
   if (SockConexion >=0) {
   		//for(t=maxh;t>0;t--){
		      ...
		      rc = pthread_create(&hilo[t], NULL, atiende , (void *)(&SockConexion[t])  );
		      
				  if( rc != 0){
				  printf("ERROR; pthread_create() = %d\n", rc);
				  }     
	    //}
        printf("Se alcanzò el lìmite de hilos");
   } 
