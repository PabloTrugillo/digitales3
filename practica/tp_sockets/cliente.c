...
int main(int argc , char const * argv[])// teneos la misa estructura de main que en el codigo de servidor
{
...
//-- preparar el address:port del host servidor------//aca va a rellenar una estructura de direccion , esta vez no para hacer un bin sino para decirle con quien se quiere conectar, al rellenarla se pone: la familia, el puerto designado arriba y ademas la direccion ip y el puerto del servidor 
...
//-- conectar el socket activo al socket de escucha --//
   conn=connect(sockfd, (struct sockaddr *)&direccion, sizeof (direccion));//los campos aca son, el descriptor y la estructura con la direccion del servidor.
