...
int des_socket, sck_server, rx_socket, largo, t=0, rc, cont;
char buffer_rx[256];
struct sockaddr_in struct_direccion={};
int SockEscucha,SockConexion,desc[5]={0};
void *hilos(void *nro)
{
	int numero=*(int*)nro;
	if (cont==0) {
	...
	while (( rx_socket = read(numero, buffer_rx, sizeof (buffer_rx))) > 0) { //lee del socket
		...
		write (STDOUT_FILENO , "cliente:--> ", 12); //escribe en pantalla(puede no ir)
