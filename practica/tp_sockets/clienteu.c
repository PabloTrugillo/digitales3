...
//****************** 1 *******************//
 //-- socket(): Crear el socket -----------//
   ...
   sockfd=socket(AF_INET, SOCK_DGRAM, 0);// en el segundo capo indicamos q es datagrama, o sea protocolo udp
...
// a diferencia del protocolo sock stream, aca la funcion connet en este caso no va xq es protocolo sin conexion
...
//-- envia al socket datos --
  while(1){ 
   ...
   //-- recibe datos del socket -- 
     ...
     lee_sck = recvfrom(sockfd, buff_sck, sizeof (buff_sck), 0,(struct sockaddr *) &direccion,&len);//espera recibir algo, q es el eco de lo q envio, la estructura a la q apunta en el ultio campo se va arellenar con la direccion de quien recive, luego lo muestra en pantalla
