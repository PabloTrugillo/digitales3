...
//al crear la fifo con mkfifo el primer argumento es el nombre y el otro los permisos 
   ...
   switch (fork()){//crea un proceso hijo
      ...
      case 0:      //Hijo
         ...
         fifo_d = open(FIFO_PATH, O_RDONLY, 0); // O_NONBLOCK // abre la fifo con permiso de solo lectura
         if(fifo_d == -1){
      ...
      default:     //Padre
         ...
         fifo_d = open(FIFO_PATH, O_WRONLY, 0);// el proc padre abre la fifo con permiso de solo escritura
         if(fifo_d == -1){
